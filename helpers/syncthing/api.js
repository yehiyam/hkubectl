...
class SyncthingApi extends EventEmitter {
    ...
    async getEvents() {
        ...
        while (true) {
            try {
                const eventList = await this._getEvents();
                eventList.filter(e => validEvents.includes(e.type)).forEach(e => {
                    this.emit('event', {
                        time: e.time,
                        type: e.type,
    ...
    async _getEvents() {
        ...
        const eventList = ret.data || [];
        if (eventList && eventList.length) {
            this._lastEventId = eventList[eventList.length - 1].id;
        }
        return eventList;
    }
}
